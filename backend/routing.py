...
from constants import (
    TRANSFER_PENALTY_DEFAULT_MIN,
    FREE_PLATFORM_TRANSFER_MIN,
)
from gtfs_processor import haversine_km as _haversine_km_gtfs
...
TRANSFER_PENALTY_MIN = TRANSFER_PENALTY_DEFAULT_MIN
                    ...
                    if dist < 200:
                        ...
                        if sid2 not in existing_from:
                            self.transfers_from.setdefault(sid1, []).append(
                                Transfer(from_id=sid1, to_id=sid2, walk_min=FREE_PLATFORM_TRANSFER_MIN)
                            )
                            self.transfers_from.setdefault(sid2, []).append(
                                Transfer(from_id=sid2, to_id=sid1, walk_min=FREE_PLATFORM_TRANSFER_MIN)
                            )
...
def _haversine_km(lat1: float, lon1: float, lat2: float, lon2: float) -> float:
    """Delegiert an gtfs_processor.haversine_km (einheitliche Implementierung)."""
    return _haversine_km_gtfs(lat1, lon1, lat2, lon2)
